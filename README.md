...
- 深色高透玻璃界面会从 Plex 中按曲目数排序的歌手背景取最多 80 张组成随机轮播池；进入歌手详情后锁定当前歌手背景且不重复铺入内容卡片，移动端 PWA 使用单行六项任务导航。
- 首页直接呈现活动 Plexamp 会话；“正在播放”统一控制本机与可验证的 Plex Companion 设备，并把原生 AirPlay 歌词投屏保持为页头主操作。
